...
## ツール実行境界（2026-06-29追加）

本文にツール呼び出し風の文字列が出ただけでは、実行したことにしない。

`<invoke name="Bash">` や `<parameter>` のような内部形式、または「Bashツールで実行します」という文章が表示されても、実ツール結果がなければ未実行として扱う。

完了扱いにするには、次のいずれかの証拠を確認する。
...
空振り、空出力、JSON parse不能、権限エラー、ツール未接続は成功ではない。再試行する場合も、freee、削除、外部投稿、自動化停止など二重実行リスクのある操作は人間確認を挟む。

詳細設計: [[tool-execution-boundary-and-script-check-wrapper]]
...
> **現状（2026-07-01 実体追記）**: agent-run 経由はまだ一部のみ（`run_vault_snapshot.sh` / `run_thread_format_learning.sh` / `vault-snapshot.sh` の3本程度）で、`run_weekly_accounting.sh` など多数の launchd/cron スクリプトは `claude -p` を直叩きしている。`agent-run` の codex 分岐は **最小配線済み**（`AGENT_VENDOR=codex agent-run -p "..."` は `codex exec` を呼ぶ）だが、`--permission-mode` / `--allowedTools` などClaude専用フラグはfail-closeするため、既存自動化の一括切替はまだ不可。上記の「agent-run 経由にする」は目標規則であり、**新規スクリプトは agent-run 経由を必須、既存スクリプトは順次移行**という移行途上の状態。詳細は [[IMPLEMENTATION_STATUS]] 参照。
...
本文にツール呼び出し風の文字列が出ただけでは実行済みにしないでください。実ツール結果・終了コード・差分・API応答などの証拠がない場合は未実行として扱ってください。
```
