...
2. `script-learning` / `daily-knowledge-extract` / `knowledge-gardener`
   ...
   - `script-learning` は2026-07-01にCodex dry-run入口を追加し、2026-07-02に本番wrapperへ手動Codex dry-run分岐を追加。さらに未承認override禁止、read-only固定、追加dir禁止、secret scan、空の一時git workdir、mtime/hash不変確認を追加。同日に通常Terminalのwrapper経由ゲートPASS。本番切替は未実施
