...
- `script-learning-codex-shadow-judgement.md`
...
4. **移行完成**
   ...
   - Codexへ要約を渡すdry-runは「外部送信なし」ではなく、「Discord投稿なし・永続書込なし・削除なし」として証拠化する
   - 構造化出力が検証器を通る
...
- Codexカスタムスキルは `/Users/kojinn/.codex/skills/<skill> -> /Users/kojinn/agent-skills/<skill>` のsymlink運用へ移行済み。2026-07-02検証で21件中21件PASS、Codex内蔵 `.system` は保持
...
- 知識はObsidian
- スキルは `~/agent-skills`
- Codexのカスタムスキル入口は `~/.codex/skills/<skill>` symlinkで `~/agent-skills/<skill>` を参照する
...
- 注記（2026-07-03）: AI運用の再構築案件に限り Commander=Fable（Claude Code）とする（`fable-command-codex-execution.md` 採用による）。本節の標準編成（Commander=Codex）は移行案件の既定として維持
...
6. Codexへ渡す入力パケットは事前要約に限定し、secret scanを通す
7. Codex `read-only` sandboxで実行し、追加dirは原則空にする
8. inner Codexのworkdirは可能な限り空の一時git repoにし、Vault全体を直接見せない
9. 構造化出力を検証する
10. 出力がない時はfail-closeする
11. 既存ファイルのmtime、hash、diff、ログ、通知有無を確認する
12. 人間承認後に手動本番相当実行を1回だけ行う
13. launchdの定期実行はデフォルトClaudeのまま、任意変数でCodexを試す
14. 1〜2周期のshadow確認後にデフォルトをCodexへ切り替える
15. Watchtower、台帳、NOW、必要ならlearning-logを更新する
...
2. `script-learning` / `daily-knowledge-extract` / `knowledge-gardener`
   ...
   - `script-learning` は2026-07-01にCodex dry-run入口を追加し、2026-07-02に本番wrapperへ手動Codex dry-run分岐を追加。さらに未承認override禁止、read-only固定、追加dir禁止、secret scan、空の一時git workdir、mtime/hash不変確認を追加。同日に通常Terminalのwrapper経由ゲートPASS。2026-08-02判定予定で1周期shadow中。本番切替は未実施
...
- Codexへ渡す要約パケットのsecret scanが通らない
- dry-runのsandboxがread-only以外、または追加dirが残っている
- 検証器が `OK` を出さない
