...
- **⚠️ 「キャッシュだから消していい」と書かれた `~/.codex/generated_images` と `~/Downloads` でも、YMM4プロジェクト(.ymmp)が素材として参照しているファイルは消さない。** 掃除の前に必ず全 `.ymmp` をチェックする（下記「掃除の鉄則」）。実例: `平将門の祟り.ymmp` は66素材のうち5個を `~/.codex/generated_images`(2) と `~/Downloads`(3) から参照していた。掃除すると再編集時に画像が消える。
...
## ⚠️ 掃除の鉄則（YouTube制作素材の保護）

`~/.codex` や `~/Downloads` を「キャッシュだから掃除していい」と判断する**前に必ず**実行する。
YMM4プロジェクト(.ymmp)は素材を**絶対パス（Windows SMBの `\\Mac\Home\...`）で参照**しており、参照先を消すと再編集時に映像が壊れる。

**掃除前チェック（参照されている素材を洗い出す）**
```
# 全 .ymmp が ~/.codex や ~/Downloads を参照していないか一覧化
find "/Volumes/SSD/YouTube" -iname "*.ymmp" -print0 | while IFS= read -r -d '' f; do
  python3 - "$f" <<'PY'
import sys, re
b = chr(92)
t = open(sys.argv[1], encoding="utf-8", errors="ignore").read().replace(b+b, b)
refs = sorted(set(re.findall(re.escape(b+"Mac"+b+"Home"+b)+r"(?:\.codex|Downloads)[^\"]+", t)))
if refs:
    print("=== "+sys.argv[1]+" ===")
...
```
**ヒットした素材は、消す前にネタフォルダ（例 `/Volumes/SSD/YouTube/平将門の祟り/`）へコピーしてから掃除する。** 理想はYMM4の「ファイルを収集」機能で素材をプロジェクト内に集約し、.ymmp を自己完結させること（→ [[AI_AGENT_STRUCTURE_AND_BACKUP]] の §6）。
**SSDのネタフォルダ自体は改名しない**（.ymmp が `\\Mac\SSD\YouTube\<ネタ名>\...` で参照しているため、改名すると全リンクが切れる）。
