...
> ⚠️ **例外**: `~/.codex/generated_images` と `~/Downloads` は原則「掃除していいキャッシュ」だが、**YMM4プロジェクト(.ymmp)が素材として参照しているファイルは消さない**（再編集時に映像が壊れる）。掃除の前に必ず `.ymmp` の参照チェックを行う（[[CLAUDE_CODE_INSTRUCTIONS]] の「掃除の鉄則」）。詳細は §8。
...
## 8. YouTube制作物の扱い（台本=Mac / 素材=SSD の分離）

1本のネタは「台本テキスト（Mac・git）」と「重い素材＋YMM4プロジェクト（SSD）」に分かれる。これは憲法どおりで**正しい分離**（素材13Gはgit/256GB Macに入らない）。ただし運用上の注意がある。

**現状の構造（実機確認・2026-06-24）**
```
台本テキスト（Mac・git）  /Projects/youtube/
   ├ 創作スレ下書き/<日付_ネタ名>/   # スレ・台本.csv/.md・各パート
   ├ 制作アーカイブ/<ネタ>_script/   # 完成台本
   └ 制作ノート/<日付_ネタ名>.md     # ネタごとに3フォルダに散在
素材＋プロジェクト（SSD）  /Volumes/SSD/YouTube/
   └ <ネタ名>/  画像・坂本龍馬暗殺の黒幕.ymmp・.mp4(完成2GB)・台本.csv
   └ 2ch世界史＿動画編集/…/背景用動画/  # 共通素材（複数ネタで共用）
```

**注意点（やってはいけないこと）**
- **SSDのネタフォルダを改名しない。** `.ymmp` が素材を絶対パス（`\\Mac\SSD\YouTube\<ネタ名>\...`）で参照しており、改名すると全リンクが切れる（平将門は58/66がネタ自身を参照）。
- **`.codex`/`Downloads` を参照素材ごと掃除しない。**（§4の⚠️）

**改善方針（段階的）**
- ① Mac側のネタフォルダに `素材 → /Volumes/SSD/YouTube/<ネタ名>` の **symlink玄関**を置く（改名せず・足すだけ。SSD接続時にワンクリックで素材へ）。
- ② 完成プロジェクトを**自己完結化**：YMM4の「ファイルを収集」で `.codex`/`Downloads` 参照素材をネタフォルダへ集約 → 掃除に強くなる。
- ③ 今後は pipeline（[[creative-thread-pipeline-設計書]] / youtube-pipeline）が生成画像を**最初からネタフォルダ直下に保存**＋完成時に収集 → 二度と散らからない。
- ネタ名は両側で同じキーに統一していく（既存は改名でなくsymlink名で吸収）。
