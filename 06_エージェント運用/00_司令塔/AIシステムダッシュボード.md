...
>
> 🎨 ビジュアル版: 同フォルダの `AIシステムダッシュボード.html`（ブラウザで開く・図やタイル付きの見やすい表示）。正本はこのmd。
...
flowchart TB
    ...
    subgraph RUN["実行レイヤー（Mac mini・24時間稼働）"]
        LD["launchd 自動化ジョブ 🟢27 / 🟡2 / 🛑4<br>結果はDiscord各チャンネルへ報告"]
        AR["agent-run＝AIベンダー差し替え口<br>（Claude / Codexを切替呼び出し・段階移行中）"]
