...
- ビジュアル版HTMLは自動生成（`20_Agent_Portable/scripts/generate_dashboard_html.py`＋`dashboard_template.html`）。HTMLだけ手で直さない。このmdのMermaid図はObsidian用で、HTML側の流れ図はテンプレ内のCSS実装
